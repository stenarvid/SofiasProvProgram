...
function useStableValues(...values) {
	...
	values.forEach((value, index) => {
		if (!(0, _tanstack_router_core.deepEqual)(stable[index], value, { ignoreUndefined: false })) stable[index] = value;
	});
	return ref.current;
}
...
function areLinkPropsEqual(prev, next) {
	...
	for (const key in next) {
		...
		if (!ROUTER_OPTION_KEYS.has(key) || !(0, _tanstack_router_core.deepEqual)(prev[key], next[key], { ignoreUndefined: false })) return false;
	}
